...
import static org.highmed.dsf.bpe.ConstantsMpcFeasibility.BPMN_EXECUTION_VARIABLE_COHORTS;
import static org.highmed.dsf.bpe.ConstantsMpcFeasibility.BPMN_EXECUTION_VARIABLE_NEEDS_CONSENT_CHECK;
import static org.highmed.dsf.bpe.ConstantsMpcFeasibility.BPMN_EXECUTION_VARIABLE_RESEARCH_STUDY;
...
public class DownloadMpcFeasibilityResources extends AbstractServiceDelegate implements InitializingBean
{
	private static final Logger logger = LoggerFactory.getLogger(DownloadMpcFeasibilityResources.class);

	private final OrganizationProvider organizationProvider;

	public DownloadMpcFeasibilityResources(FhirWebserviceClientProvider clientProvider, TaskHelper taskHelper,
	...
	private IdType getResearchStudyId(Task task)
	{
		Reference researchStudyReference = getTaskHelper().getInputParameterReferenceValues(task,
				CODESYSTEM_HIGHMED_MPCMPCFEASIBILITY, CODESYSTEM_HIGHMED_MPCMPCFEASIBILITY_VALUE_RESEARCH_STUDY_REFERENCE)
				.findFirst().get();
	...
	private boolean getNeedsConsentCheck(Task task)
	{
		return getTaskHelper()
				.getFirstInputParameterBooleanValue(task, CODESYSTEM_HIGHMED_MPCFEASIBILITY,
						CODESYSTEM_HIGHMED_MPCFEASIBILITY_VALUE_NEEDS_CONSENT_CHECK)
