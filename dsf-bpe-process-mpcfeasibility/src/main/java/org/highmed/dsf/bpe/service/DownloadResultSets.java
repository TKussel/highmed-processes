...
public class DownloadResultSets extends AbstractServiceDelegate
{
	...
	protected void doExecute(DelegateExecution execution) throws Exception
	{
		...
		List<MpcFeasibilityQueryResult> resultsWithResultSets = download(results);

		execution.setVariable(BPMN_EXECUTION_VARIABLE_QUERY_RESULTS,
				MpcFeasibilityQueryResultsValues.create(new MpcFeasibilityQueryResults(resultsWithResultSets)));
	}

	private List<MpcFeasibilityQueryResult> download(MpcFeasibilityQueryResults results)
	{
		return results.getResults().stream().map(r -> download(r)).collect(Collectors.toList());
	}

	private MpcFeasibilityQueryResult download(MpcFeasibilityQueryResult result)
	{
		...
		return MpcFeasibilityQueryResult.idResult(result.getOrganizationIdentifier(), result.getCohortId(), resultSet);
	}
