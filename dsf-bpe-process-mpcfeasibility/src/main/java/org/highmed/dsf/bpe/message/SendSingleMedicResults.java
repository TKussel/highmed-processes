...
public class SendSingleMedicResults extends AbstractTaskMessageSend
{
	...
	private Task.ParameterComponent toInput(MpcFeasibilityQueryResult result)
	{
		if (result.isCohortSizeResult())
		{
			ParameterComponent input = getTaskHelper().createInputUnsignedInt(CODESYSTEM_HIGHMED_MPCFEASIBILITY,
					CODESYSTEM_HIGHMED_MPCFEASIBILITY_VALUE_SINGLE_MEDIC_RESULT, result.getCohortSize());
			input.addExtension(createCohortIdExtension(result.getCohortId()));
			return input;
		}
		else if (result.isIdResultSetUrlResult())
		{
			ParameterComponent input = getTaskHelper().createInput(CODESYSTEM_HIGHMED_MPCFEASIBILITY,
					CODESYSTEM_HIGHMED_MPCFEASIBILITY_VALUE_SINGLE_MEDIC_RESULT_REFERENCE,
