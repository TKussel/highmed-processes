...
public class CheckSingleMedicResults extends AbstractServiceDelegate
{
	...
	protected void doExecute(DelegateExecution execution) throws Exception
	{
		...
		List<MpcFeasibilityQueryResult> filteredResults = filterErroneousResultsAndAddErrorsToCurrentTaskOutputs(results,
				currentTask);
	...
	private List<MpcFeasibilityQueryResult> filterErroneousResultsAndAddErrorsToCurrentTaskOutputs(
			MpcFeasibilityQueryResults results, Task task)
	{
		List<MpcFeasibilityQueryResult> filteredResults = new ArrayList<>();
		for (MpcFeasibilityQueryResult result : results.getResults())
		{
	...
	protected Optional<String> testResultAndReturnErrorReason(MpcFeasibilityQueryResult result)
	{
	...
	{
		String errorMessage = "MpcFeasibility query result check failed for group with id '" + cohortId + "': " + error;
