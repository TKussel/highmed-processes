...
import static org.highmed.dsf.bpe.ConstantsMpcFeasibility.CODESYSTEM_HIGHMED_FEASIBILITY;
import static org.highmed.dsf.bpe.ConstantsMpcFeasibility.CODESYSTEM_HIGHMED_FEASIBILITY_VALUE_MULTI_MEDIC_RESULT;
import static org.highmed.dsf.bpe.ConstantsMpcFeasibility.CODESYSTEM_HIGHMED_FEASIBILITY_VALUE_PARTICIPATING_MEDICS_COUNT;
...
public class CheckMultiMedicResults extends AbstractServiceDelegate
{
	...
	protected void doExecute(DelegateExecution execution) throws Exception
	{
		...
		addFinalMpcFeasibilityQueryErrorsToLeadingTask(currentTask, leadingTask);

		FinalMpcFeasibilityQueryResults results = readFinalMpcFeasibilityQueryResultsFromCurrentTask(currentTask);
		FinalMpcFeasibilityQueryResults checkedResults = checkResults(results);

		addFinalMpcFeasibilityQueryResultsToLeadingTask(checkedResults, leadingTask);
	...
	private void addFinalMpcFeasibilityQueryErrorsToLeadingTask(Task toRead, Task toWrite)
	{
	...
	private FinalMpcFeasibilityQueryResults readFinalMpcFeasibilityQueryResultsFromCurrentTask(Task task)
	{
		List<FinalMpcFeasibilityQueryResult> results = task.getInput().stream()
				.filter(in -> in.hasType() && in.getType().hasCoding()
		...
		return new FinalMpcFeasibilityQueryResults(results);
	}

	private FinalMpcFeasibilityQueryResult toResult(Task task, ParameterComponent in)
	{
		...
		return new FinalMpcFeasibilityQueryResult(cohortId, participatingMedics, cohortSize);
	}
	...
	protected FinalMpcFeasibilityQueryResults checkResults(FinalMpcFeasibilityQueryResults results)
	{
	...
	private void addFinalMpcFeasibilityQueryResultsToLeadingTask(FinalMpcFeasibilityQueryResults results, Task toWrite)
	{
		results.getResults().forEach(result -> addResultOutput(result, toWrite));
	}

	private void addResultOutput(FinalMpcFeasibilityQueryResult result, Task toWrite)
	{
