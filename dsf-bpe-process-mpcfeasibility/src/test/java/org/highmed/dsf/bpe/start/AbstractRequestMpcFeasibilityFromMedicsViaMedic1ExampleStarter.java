...
public abstract class AbstractRequestMpcFeasibilityFromMedicsViaMedic1ExampleStarter
{
	...
	private Task createTask(ResearchStudy researchStudy)
	{
		...
		task.addInput()
				...
				.getType().addCoding().setSystem(CODESYSTEM_HIGHMED_MPCFEASIBILITY)
				.setCode(CODESYSTEM_HIGHMED_MPCFEASIBILITY_VALUE_RESEARCH_STUDY_REFERENCE);
		task.addInput().setValue(new BooleanType(true)).getType().addCoding().setSystem(CODESYSTEM_HIGHMED_MPCFEASIBILITY)
